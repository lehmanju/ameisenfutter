...
public class Launcher extends Application implements EventHandler<WorkerStateEvent> {
  ...
  /*
   * Sobald Simulation zu Ende zeichne Grafiken auf Bildschirm
   */
  @Override
  ...
  public void changeSettings() {
    ...
    nestXF.setPromptText(String.valueOf(sp.nestX));
    nestYF.setPromptText(String.valueOf(sp.nestY));
    ...
    confirm.showAndWait().ifPresent(result -> {
      if (result == ButtonType.OK) {
        try {
          ...
          boolean groesserNull =
              newAmeisen > 0 && newGroesse[0] > 0 && newGroesse[1] > 0 && newNestP[0] > 0
  ...
  public void reset(int ameisen, int[] groesse, int[] nest, int futterQ, int timeout,
      int iterations) {
    sp = new Speicher(futterQ, ameisen, 50, groesse[0], groesse[1], nest[0], nest[1], timeout,
        iterations);
    area = new DrawArea(titledPane, sp, width);
    Simulator sim = new Simulator(sp);
    simService = new SimulationService(sim, sp, width);
